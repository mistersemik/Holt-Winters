...
- Критические ошибки в функциях:
  - `hw_bayesian_ensemble`:
    - Исправлена ошибка распаковки значений в байесовской 
      модели
    - Оптимизированы параметры сэмплирования
    - Добавлена проверка входных данных
  - `wavelet_hw`:
    - Полностью переработана логика вейвлет-преобразования
    - Исправлена ошибка "coefficient shape mismatch"
    - Добавлена адаптивная обработка коротких временных
      рядов
    - Улучшен механизм fallback при ошибках

### Улучшено

- Общая стабильность работы всех моделей:
  - Автоматическая обработка временных рядов без
    явной частоты
  - Защита от ошибок при недостаточном количестве данных
  - Унифицированные сообщения об ошибках
- Производительность:
  - Оптимизировано использование памяти
  - Ускорено вейвлет-преобразование
  - Снижена нагрузка на CPU

### Изменено

- Логика работы с внешними признаками в
  `hw_xgboost_ensemble`
- Требования к минимальной длине временного ряда
  (24 периода вместо 36)
- Формат возвращаемых прогнозов
  (гарантированно pandas.Series с DatetimeIndex)
...
- - Гибридная модель `hw_sarima_ks` (SARIMA + Kernel Smoothing)
- Поддержка ARCH-моделей через библиотеку `arch`
- Интеграция `statsmodels.nonparametric` для ядерного сглаживания
- Автоматический fallback-механизм для всех новых моделей
